import { ALT_COLORS, COLORS, ITEM_KINDS, PLAYER, SONAR, TILE, type ItemKind } from '@/config';
...
export class Renderer {
  ...
  private drawWorld(ctx: CanvasRenderingContext2D, f: Floor, alpha: number, dtFrame: number, hud: HudState, live: boolean): void {
    ...
    const applyWorld = (c: CanvasRenderingContext2D, scaleMul: number) => {
      c.setTransform(this.dpr * scaleMul, 0, 0, this.dpr * scaleMul, 0, 0);
  ...
  private drawPulseWalls(ctx: CanvasRenderingContext2D, f: Floor, t: number, inView: (x: number, y: number) => boolean): void {
    ...
    const BUCKETS = 8;
    const key = (colorIdx: number, b: number) => colorIdx * BUCKETS + b;
    const colorList: RGB[] = [];
  ...
  private runEndFor(pulse: Pulse, k: number): number {
    ...
    const start = runs[k]!;
    const next = runs[k + 1]!;
    // runs are contiguous except where misses split them; walk forward while consecutive rays are hits with matching side
    const hits = pulse.hits;
    let j = start;
    while (j + 1 < next) {
      const a = j * 4;
  ...
  private drawBlip(ctx: CanvasRenderingContext2D, b: Blip, a: number, t: number): void {
    ...
    switch (b.kind) {
      ...
      case 'predator': {
        ...
        ctx.strokeStyle = rgba(b.kind === 'predator' ? COLORS.predator : COLORS.hunter, a);
        ctx.fillStyle = rgba(b.kind === 'predator' ? COLORS.predator : COLORS.hunter, a * 0.25);
        ctx.beginPath();
        ctx.moveTo(x, y - s);
        ctx.lineTo(x + s, y);
        ctx.lineTo(x, y + s);
      ...
      case 'item': {
        ...
        ctx.stroke();
        if (b.label) {
          ctx.fillStyle = rgba(COLORS.item, a);
          ctx.font = '9px system-ui, sans-serif';
          ctx.textAlign = 'center';
          ctx.fillText(S.itemName[b.label as ItemKind] ?? '', x, y + 15);
        }
        break;
      }
      case 'key': {
  ...
  private drawHud(ctx: CanvasRenderingContext2D, sim: GameSim, f: Floor, hud: HudState): void {
    ...
    // floor / timer
    ctx.font = '600 15px system-ui, "Apple SD Gothic Neo", "Malgun Gothic", sans-serif';
    ctx.textAlign = 'center';
    ...
    if (p.hasKey) {
      ctx.textAlign = 'right';
      ctx.fillStyle = rgba(COLORS.key, 0.95);
      ctx.font = '600 13px system-ui, sans-serif';
      ctx.fillText('열쇠 ●', w - 18, 20);
    } else if (f.layout.locked) {
      ctx.textAlign = 'right';
      ctx.fillStyle = 'rgba(255,255,255,0.35)';
      ctx.font = '600 13px system-ui, sans-serif';
      ctx.fillText('출구 잠김', w - 18, 20);
    }
    // silencer bar
    if (p.silencerT > 0) {
    ...
    if (!hud.touch) {
      ...
      ITEM_KINDS.forEach((k, i) => {
        ...
        ctx.font = '600 12px system-ui, "Apple SD Gothic Neo", "Malgun Gothic", sans-serif';
        ctx.textAlign = 'left';
        ctx.fillText(`${i + 1}`, x + 12, y - 5);
        ctx.font = '12px system-ui, "Apple SD Gothic Neo", "Malgun Gothic", sans-serif';
    ...
    // hint / toast
    ctx.textAlign = 'center';
    if (hud.toast && hud.toastT > 0) {
      ctx.font = '600 13px system-ui, "Apple SD Gothic Neo", "Malgun Gothic", sans-serif';
      ctx.fillStyle = `rgba(230,240,255,${Math.min(1, hud.toastT)})`;
      ctx.fillText(hud.toast, w / 2, h / 2 - 60);
    }
    if (hud.hint) {
      const y = hud.touch ? 64 : h - 70;
      ctx.font = '13px system-ui, "Apple SD Gothic Neo", "Malgun Gothic", sans-serif';
